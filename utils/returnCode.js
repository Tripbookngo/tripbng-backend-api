...
function returnCode(url, header, void_fail_message, fail_message, success_message) {
  return asyncHandler(async (req, res) => {
    ...
    if (!data) {
      return res
        .status(400)
        .json(new ApiResponse(401, { success: false, data: void_fail_message }, void_fail_message));
    }
    ...
    if (!result) {
      return res
        .status(400)
        .json(new ApiResponse(401, { success: false, data: fail_message }, fail_message));
    }
    return res
      .status(200)
      .json(new ApiResponse(201, { success: true, data: result.data }, success_message));
  });
}
