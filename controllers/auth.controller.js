...
const login = asyncHandler(async (req, res) => {
    ...
    if (contact_feild.includes("@")) {
        ...
        // try {
        await sendMail(contact_feild, "Your TripBnG Verification Code", `Hi ${contact_feild},
...
TripBnG India Private Limited`);
        ...
        try {
            await sendSMS(`Hi ${contact_feild},
...
Safe travels,
TripBnG India Private Limited`, contact_feild);
        } catch (err) {
...
const verifyOTP = asyncHandler(async (req, res) => {
    ...
    const saveIpDetails = await LoginDetails.create({ ip: ipdetails.ip, logintime: ipdetails.logintime, browserdetails: ipdetails.browserdetails })
...
const deleteAccountOtpsend = asyncHandler(async (req, res) => {
    ...
    if (!mobile) {
        return res.status(400)
            .json(
                new ApiResponse(400, { success: false, data: "Mobile number is not fecthed" }, "Mobile number is not fecthed")
            )
    }
    ...
    if (mobile !== user.mobile) {
        return res.status(400)
            .json(
                new ApiResponse(400, { success: false, data: "Mobile number is not valid" }, "Mobile number is not valid")
            )
    }
...
const ForgetPassword = asyncHandler(async (req, res) => {
    ...
    if(usermail !=  user.email){
        return res.status(400)
        .json(
            new ApiResponse(400,{success:false,data:"this is not correct email id"},"this is not correct email id")
        )
    }


    const code = generateOTP();
    await sendMail(user.email, "Reset Your TripBNG Password", `Hi ${user.email},
...
const GetOtpOfLogoutPasswordForget = asyncHandler(async(req,res)=>{
    ...
    await sendMail(email, "Reset Your TripBNG Password", `Hi ${email},
