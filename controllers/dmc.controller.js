  ...
  try {
    ...
    if (!dmcWithEmail) {
      ...
      if (!dmcWithMobile) {
        return res
          .status(400)
          .json(errorMessage('Account does not exists. Please register first.'));
      } else {
        if (!dmcWithMobile.adminApproved) {
          return res.status(400).json(errorMessage('Account is currently under admin approval.'));
        }
  ...
  try {
    ...
    if (!dmcWithEmail) {
      ...
      if (!dmcWithMobile) {
        ...
        const token = jwt.sign({ id: dmcWithMobile._id }, process.env.JWT_SECRET);
        return res.status(200).json(successMessage('OTP Verified', { token, data: dmcWithMobile }));
      }
    } else {
      if (!dmcWithEmail.adminApproved) {
        return res.status(400).json(errorMessage('Account is currently under admin approval.'));
      ...
      return res.status(200).json(successMessage('OTP Verified', { token, data: dmcWithEmail }));
    }
  } catch (err) {
    return res.status(500).json(errorMessage(err?.message || 'Something went wrong'));
  }
...
const verifyEmail = async (req, res) => {
  try {
    ...
    if (!data) {
      return res.status(400).json(errorMessage('Invalid OTP, Request a new one.'));
    }
...
const registerDMC = async (req, res) => {
  try {
    ...
    return res.status(200).json(successMessage('Registered Successfully', data));
  } catch (err) {
    return res.status(500).json(errorMessage(err?.message || 'Something went wrong'));
  }
};
...
const createPackage = asyncHandler(async (req, res) => {
  try {
    ...
    return res
      .status(201)
      .json(
        new ApiResponse(201, { success: true, data: newPackage }, 'Package created successfully')
      );
  } catch (error) {
    ...
    return res
      .status(500)
      .json(
        new ApiResponse(500, { success: false, error: error.message }, 'Internal Server Error')
      );
  }
});
