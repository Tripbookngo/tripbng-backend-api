...
const searchPackages = asyncHandler(async (req, res) => {
  ...
  if (!saveData) {
    return res
      .status(400)
      .json(new ApiResponse(400, { sucess: false, data: 'Please Entet the all data' }));
  }
  return res
    .status(200)
    .json(
      new ApiResponse(
        ...
        'Your Holiday request package has been send successfully!'
      )
    );
});
...
const createPackages = asyncHandler(async (req, res) => {
  ...
  if (isNull([destination, nights, days, hotel_type, discription])) {
    return res
      ...
      .json(
        new ApiResponse(
          400,
          { success: false, data: 'Please Enter  the all data' },
          'Please Enter the all data'
        )
      );
  }
  ...
  if (!CreateNewPackages) {
    return res
      ...
      .json(
        new ApiResponse(
          400,
          { success: false, data: 'Sorry packages is not created' },
          'sorry packages is not created'
        )
      );
  }
  ...
  return res
    ...
    .json(
      new ApiResponse(
        200,
        { success: true, data: 'Your packages is created we response you in message' },
        'data padh lo bhai'
      )
    );
});

export { createPackages, searchPackages };
