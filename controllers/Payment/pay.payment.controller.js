...
const payUsuccess = asyncHandler(async (req, res) => {
  ...
  if (
    ...
    return res
      .status(400)
      .json(new ApiResponse(400, null, 'Payment verification failed or transaction not found'));
  }
  ...
  if (!isTxnExist) {
    const for_payment = data.productinfo
      .replaceAll('quot', '')
      .replaceAll('type', '')
      .replaceAll(' ', '')
      .replaceAll('  ', '')
  ...
  res.status(200).json(new ApiResponse(200, { success: true, data }, 'Payment is done'));
});
...
const payUPayment = asyncHandler(async (req, res) => {
  ...
  const data = await PayUPaymentScrater(
    amount,
    { type: type },
    firstname,
    lastname,
