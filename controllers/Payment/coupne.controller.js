...
const GenerateCoupunCode = () => {
  ...
  const coupen =
    ...
    alfabates
      .slice(
        Math.floor(Math.random() * (13 - 0 + 1) + 0),
        Math.floor(Math.random() * (26 - 13 + 1) + 13)
      )
      .slice(0, 2) +
    smallAlfaBates
      .slice(
...
const CreateCoupne = asyncHandler(async (req, res) => {
  ...
  if (!dataSave) {
    return res
      ...
      .json(
        new ApiResponse(
          200,
          { success: false, data: 'Something problem in the Server site' },
          'Something Problem At server site'
        )
      );
  }
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, { success: true, data: createCoupne }, 'coupen code is generated'));
});
...
const UseCoupne = asyncHandler(async (req, res) => {
  ...
  if (!coupen_code) {
    return res
      ...
      .json(
        new ApiResponse(
          401,
          { success: false, data: 'please Enter the coupen code' },
  ...
  if (!isCoupenValid) {
    return res
      ...
      .json(
        new ApiResponse(
          401,
          { sucess: false, data: 'please enter the valid coupen code' },
          'please enter the valid coupen code'
        )
      );
  }
  ...
  if (!isCoupenValid.isUsable) {
    return res
      ...
      .json(
        new ApiResponse(
          401,
          { sucess: false, data: 'this coupen is already used' },
  ...
  if (finalAmount < 0) {
    return res
      ...
      .json(
        new ApiResponse(
          401,
          { success: false, data: 'this coupen is not aplicable for this item' },
          'this coupen is not aplicable for the item'
        )
      );
  }
  ...
  return res
    .status(200)
    .json(new ApiResponse(201, { success: true, data: finalAmount }, 'Your final amount discount'));
});
  ...
  if (!isCoupenValid) {
    return res
      ...
      .json(
        new ApiResponse(
          401,
          { success: false, data: 'please enter the valid coupen code' },
          'please enter the valid coupen code'
        )
      );
  }
...
const GetAllCoupenCode = asyncHandler(async (req, res) => {
  ...
  if (findcoupen.length == 0) {
    return res
      ...
      .json(
        new ApiResponse(
          201,
          { success: true, data: "user does't have any coupen" },
          "user don't have any coupen"
        )
      );
  }
  ...
  return res
    .status(200)
    .json(new ApiResponse(201, { success: true, data: findcoupen }, 'all the coupen data'));
});

export { CreateCoupne, UseCoupne, DisableCoupen, GetAllCoupenCode };
