...
const GetBalance = asyncHandler(async (req, res) => {
  const CheckPyment = await sendPostRequest(
  ...
  if (!CheckPyment) {
    return res
      .status(400)
      .json(new ApiResponse(400, { success: false, data: 'Authfail' }, 'Auth Fail'));
  }
...
const AddBusPayment = asyncHandler(async (req, res) => {
  ...
  if (isNull([ClientRefNo, RefNo, TransactionType, ProductId])) {
    return res
      .status(400)
      .json(new ApiResponse(400, { success: false, data: 'Please Enter All field' }));
  }

  const AddPayment = await sendPostRequest(
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, { success: true, data: AddPayment }, 'Payment Add successfully'));
});
  ...
  if (isNull([ClientRefNo, RefNo, TransactionType, ProductId])) {
    return res
      ...
      .json(
        new ApiResponse(
          400,
          { success: false, data: 'Please Enter All field Complsry' },
          'Please Add All field complsry'
        )
      );
  }
