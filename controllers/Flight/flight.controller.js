...
const GetAllBestFlight = asyncHandler(async (req, res) => {
  ...
  const DataHolder = [
    Api1Data?.data?.TripDetails ?? null,
    Api2Data?.data?.searchResult?.tripInfos ?? null,
  ];
  ...
  if (Data.isReturn) {
    ...
    if (!FlightOnward && !FlightReturn) {
      return res
        .status(200)
        .json(
          new ApiResponse(200, { success: true, data: 'No flight data found' }, 'No flights found')
        );
    }
    ...
    if (!response) {
      return res
        .status(200)
        .json(
          new ApiResponse(200, { success: false, data: 'No flights found' }, 'No flights found')
        );
    }
    //send the response to the client
    return res.status(200).json(
...
const SearchAirLine = asyncHandler(async (req, res) => {
  try {
    ...
    return res.status(200).json(new ApiResponse(200, { success: true, data }, 'Data Fetch'));
  } catch (err) {
    return res.status(500).json(new ApiResponse(500, { success: false }, 'Fail to fetch AirLine'));
  }
});
  ...
  if (ApiNo == 1) {
    try {
      if (isNull([FareId, SearchKey, FlightKey, ApiNo])) {
        return res
          .status(400)
          .json(new ApiResponse(400, { success: false }, 'please Enter All the Feild Compalsary'));
      }
      ...
      console.log(error);
      return res.status(400).json(new ApiResponse(400, { success: false, data: error }, error));
    }
  }
...
const GetFlightSeat = asyncHandler(async (req, res) => {
  ...
  if (ApiNo == '1') {
    if (!Api1Data) {
      return res
        .status(400)
        .json(new ApiResponse(400, { success: false }, 'Please Enter the Api 1 request Data'));
    }
  ...
  if (ApiNo == '2') {
    ...
    if (bookingid == 'Keys Passed in the request is already expired. Please pass valid keys') {
      return res.status(400).json(new ApiResponse(400, { success: false }, bookingid));
    }

    //if we get the Booking Id then get the SeatMap using bookingId
    const seatMap = await sendPostRequest('https://apitest.tripjack.com/fms/v1/seat', head, {
      bookingId: bookingid,
    });
    ...
    if (seatMap.errors) {
      return res
        .status(200)
        .json(new ApiResponse(200, { success: false }, seatMap.errors[0].message));
    }
...
const GetFlightSSR = asyncHandler(async (req, res) => {
  ...
  if (ApiNo == '1') {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, { success: true, data: SSRDetails }, 'Successfully fetched'));
  }
  if (ApiNo == '2') {
    return res
      .status(200)
...
const ReviewSection = asyncHandler(async (req, res) => {
  const { id } = req.body;
  const reviewData = await sendPostRequest(`https://apitest.tripjack.com/fms/v1/review`, head, {
    priceIds: [id],
  });

  return res
    .status(200)
    .json(new ApiResponse(200, { success: true, data: reviewData }, ` data is ${reviewData}`));
});
  ...
  if (ApiNo == 1) {
    ...
    if (!charges.data) {
      return res
        .status(200)
        .json(
          new ApiResponse(200, { success: false, data: 'charges not found' }, 'charges not found')
        );
    }
...
const FlightBooking = asyncHandler(async (req, res) => {
  ...
  if (body.ApiNo == '1') {
    ...
    return res.status(200).json(new ApiResponse(200, { success: true, data }, 'Success'));
    ...
    const PNR = await sendPostRequest(`https://apitest.tripjack.com/oms/v1/booking-details`, head, {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, { success: true, data: data.data || data }, 'Booking Done'));
  }
});
...
const GetBookingDetails = asyncHandler(async (req, res) => {
  ...
  if (!ApiNo) {
    return res
      .status(400)
      .json(new ApiResponse(400, { success: false, data: 'Please Enter the ApiNo' }));
  }
  ...
  if (ApiNo == '1') {
    ...
    return res
      .status(200)
      .json(
        new ApiResponse(200, { success: true, data: bookingDetails.data }, 'Booking SuccessFull')
      );
  } else if (ApiNo == '2') {
    try {
      const Bookingdetails = await sendPostRequest(
      ...
      return res
        .status(400)
        .json(new ApiResponse(400, { success: false, data: error }, `error is come ${error}`));
    }
  }
});
...
const CancelFlightBooking = asyncHandler(async (req, res) => {
  ...
  if (ApiNo == '1') {
    try {
      ...
      return res
        .status(400)
        .json(new ApiResponse(400, { success: false, data: error }, `error is occure ${error}`));
    }
  }
  if (ApiNo == '2') {
    const { id } = Api2Data;
    ...
    if (!id) {
      return res
        .status(400)
        .json(new ApiResponse(400, { success: false, data: 'please enter id' }, 'please Enter Id'));
    }

    const cancelFlightApi2 = await sendPostRequest(
      `https://apitest.tripjack.com/oms/v1/hotel/cancel-booking/${id}`,
      head
    );
    ...
    if (cancelFlightApi2.data.errors) {
      return res
        ...
        .json(
          new ApiResponse(
            400,
            { success: true, data: cancelFlightApi2.data },
            'Booking unCancelSuccessfully'
          )
        );
    }
...
const Reprice = asyncHandler(async (req, res) => {
  const { ApiNo, Api1Data, Api2Data } = req.body;
  if (!ApiNo) {
    return res.status(400).json(new ApiResponse(400, { success: false, data: 'no api no' }, ''));
  }
  ...
  if (ApiNo == '1') {
    ...
    if (!data) {
      return res
        .status(400)
        .json(new ApiResponse(400, { success: false, data: 'not come data' }, ''));
    }

    return res.status(200).json(new ApiResponse(200, { success: true, data: data }, 'success'));
  } else if (ApiNo == 2) {
    const data = await sendPostRequest('https://apitest.tripjack.com/fms/v1/review', headers, {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, { success: true, data: data.data }, 'data fetch successfully!'));
  }
});
...
const MakrupData = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, { success: true, data: markup }, 'markup amount'));
});

const AddBalace = asyncHandler(async (req, res) => {
  const { RefNo } = req.body;
  const add = await sendPostRequest(
    'https://api.etrav.in/Trade/TradeAPIService.svc/JSONService/AddPayment',
    {},
    {
      Auth_Header: authHeaders(),
  ...
  return res.status(200).json(new ApiResponse(200, { success: true, data: add.data }, 'success'));
});
...
const Tickting = asyncHandler(async (req, res) => {
  ...
  try {
    const tick = await sendPostRequest(
      `${process.env.LIVE_URL_ETRAV_FLIGHT}/AirAPIService.svc/JSONService/Air_Ticketing`,
      {},
      {
        Auth_Header: authHeaders(),
    ...
    if (tick.data.AirlinePNRDetails.AirlinePNRs) {
      const getFlightData = await FlightBooking.findOne({
        UserId: req.user._id,
        BookingRefNum: Booking_RefNo,
      });
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, { success: true, data: tick.data }, 'success'));
  } catch (error) {
    return res
      .status(400)
      .json(new ApiResponse(400, { success: false, data: error }, 'somthing error is coming'));
  }
});
