...
const UploadVisaFile = async (req, res) => {
  try {
    if (req.file) {
      ...
      return res.status(200).json(successMessage('Upload Successfully', { data: unsignedUrl }));
    } else {
...
const getPreSignedUrl = async (req, res) => {
  try {
    ...
    const bucketName = unsignedUrl.split('.s3.amazonaws.com/')[0].split('https://')[1];
  ...
  if (!username) {
    return res
      ...
      .json(
        new ApiResponse(
          400,
          { success: false, data: 'Please Enter Username' },
  ...
  if (!DataOfBusBooking) {
    return res
      ...
      .json(
        new ApiResponse(
          200,
          { success: false, data: 'User not book any bus tickit' },
          'user not book any bus tickit'
        )
      );
  }
...
const GetFlightBookingHistroy = asyncHandler(async (req, res) => {
  ...
  if (flightbookingdatahistroy.length == 0) {
    return res
      ...
      .json(
        new ApiResponse(
          200,
          { success: true, data: 'use not book any flight tickit' },
          'User not book any flight '
        )
      );
  }
  ...
  return res
    ...
    .json(
      new ApiResponse(
        200,
        { success: true, data: flightbookingdatahistroy },
        'flight booking data fetch successfuly'
      )
    );
});
...
const VisaQuiryGenrate = asyncHandler(async (req, res) => {
  ...
  if (!SaveData) {
    return res
      .status(400)
      .json(new ApiResponse(400, { success: false, data: 'Please Enter the Hole data' }, 'hmm'));
  }
  console.log(req.user);
  return res
    .status(200)
    .json(
      new ApiResponse(
        ...
        'Your visa request has been send successfully!'
      )
    );
});
...
const GetVolateBalance = asyncHandler(async (req, res) => {
  return res
    ...
    .json(
      new ApiResponse(
        200,
        { success: true, data: req.user.volate },
        `Your volate amount is ${req.user.volate}`
      )
    );
});
  ...
  try {
    ...
    return res
      ...
      .json(
        new ApiResponse(
          200,
          { success: true, data: user.volate },
          `Your total amount is ${user.volate}`
        )
      );
  } catch (error) {
    return res
      ...
      .json(
        new ApiResponse(
          400,
          { success: false, data: error.message || error },
...
const WithdrawVolate = asyncHandler(async (req, res) => {
  ...
  if (amount > user_amount) {
    return res
      ...
      .json(
        new ApiResponse(
          200,
          { success: false, data: 'Your Balance is not inuf' },
          'Your Balance is not inuf'
        )
      );
  }
  const user = req.user;
  ...
  return res
    ...
    .json(
      new ApiResponse(
        200,
        { success: false, data: `successfully withdraw! now your balance is ${user.volate}` },
        `successfully withdraw! now your balance is ${user.volate}`
      )
    );
});
...
const GetAllTrasactions = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, { success: true, data: trans }, 'data fetch successfully'));
});
export default {
  getProfile,
  updateProfile,
