...
export const createAirlineMarkup = AsnycHandler(async (req, res) => {
    ...
    if (isNull([carrier])) {
        return res.status(400).json(
            new ApiResponse(400, { success: false, data: "Carrier is required" })
        );
    }

    const segments = [domestic_one_way, domestic_round_way, international_one_way, international_round_way];
    for (const segment of segments) {
        if (segment?.commision_type !== undefined && !isValidCommisionIndex(segment.commision_type)) {
            return res.status(400).json(
                new ApiResponse(400, { success: false, data: "Invalid commision_type index" })
            );
        }
    }
...
export const getAllAirlineMarkups = AsnycHandler(async (req, res) => {
    ...
    return res.status(200).json(
        new ApiResponse(200, { success: true, data: formatted }, "Fetched all markups")
    );
});
