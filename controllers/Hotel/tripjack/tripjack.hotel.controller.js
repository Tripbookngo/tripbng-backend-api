...
const cancellation = returnCode(
  'https://apitest.tripjack.com/hms/v1/hotel-cancellation-policy',
  headers,
  'please enter all the feilds',
  'please enter correct feilds',
  'data fetch successfully'
);

const review = returnCode(
  'https://apitest.tripjack.com/hms/v1/hotel-review',
  headers,
  'please enter all the ids',
  'please enter all correct ids',
  'data fetch successfully'
);
...
const booking = asyncHandler(async (req, res) => {
  ...
  if (bookingObj == {}) {
    return res
      ...
      .json(
        new ApiResponse(
          401,
          { success: false, data: 'please enter entire object' },
          'please enter entire object'
        )
      );
  }
  ...
  if (!result) {
    return res
      ...
      .json(
        new ApiResponse(
          401,
          { success: false, data: 'please enter correct details' },
          'please eneter correct details'
        )
      );
  }
  ...
  return res
    .status(200)
    .json(new ApiResponse(201, { success: true, data: result.data }, 'data fetch successfully'));
});
...
const confirmHoldBooking = asyncHandler(async (req, res) => {
  ...
  if (!paymentInfos) {
    return res
      ...
      .json(
        new ApiResponse(
          401,
          { success: false, data: 'please give us hole payment information' },
  ...
  if (!result) {
    return res
      ...
      .json(
        new ApiResponse(
          400,
          { success: false, data: 'please enter correct data' },
          'please enter correct data'
        )
      );
  }
  ...
  return res
    .status(200)
    .json(new ApiResponse(201, { success: true, data: result.data }, 'fetch the all data'));
});

const bookingDetails = returnCode(
  'https://apitest.tripjack.com/oms/v1/hotel/booking-details',
  headers,
  'please enter booking id',
  'please enter correct booking id',
  'data fetch successully'
);
  ...
  if (!bookingId) {
    return res
      ...
      .json(
        new ApiResponse(
          400,
          { success: false, data: 'please enter the booking_id' },
