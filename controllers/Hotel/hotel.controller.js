...
const HotelBooking = asyncHandler(async (req, res) => {
  ...
  if (!hotelid) {
    return res
      ...
      .json(
        new ApiResponse(
          400,
          { success: false, data: 'Hotel ID is required' },
  ...
  if (isNull(body)) {
    return res
      .status(400)
      .json(new ApiResponse(400, { success: false, data: 'Body is required' }, 'Body is required'));
  }

  const booking = await sendPostRequest(
    `${process.env.DEMO_HOTEL_URL}/${hotelid}/${token}/book`,
    header,
    { ...body }
  );
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, { success: true, data: booking.data }, 'Data fetched'));
});
  ...
  if (!channelId || !bookingId) {
    return res
      ...
      .json(
        new ApiResponse(
          400,
          { success: false, data: 'Please Enter the Channel ID and Booking ID' },
          'Please Enter the Channel ID and Booking ID'
        )
      );
  }

  const hotelDetails = await sendGetRequest(
    `${process.env.DEMO_HOTEL_URL}/getBookingDetails`,
    header,
    { channelId, bookingId }
  );
});

export { HotelBooking, GetHotelDetails };
